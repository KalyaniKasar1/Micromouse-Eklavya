...
A micromouse is a small, but fast bot that can find its way from the start position of a 16x16 wooden maze to its center, unaided, in a matter of seconds. The mouse will need to keep track of where it is, discover walls as it explores, map out the maze and detect when it has reached the goal. 
The micromouse is allowed 7 minutes and 7 dry runs to map the maze, find the path to the centre, memorise the path and then make the fastest possible run to the centre. The mouse with the record of the shortest time wins! :smiley:
